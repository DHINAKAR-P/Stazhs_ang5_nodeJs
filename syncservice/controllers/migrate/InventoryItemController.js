...
const sequelize = new Sequelize(config.dumpDb.name, config.dumpDb.username, config.dumpDb.password, {
  ...
  module.exports.migrateInventoryItem = function () {
    ...
    sequelize.query("SELECT TOP 1 * FROM P_FinishedGoodsAdjustments ORDER BY DataExportDate DESC",
        { type: sequelize.QueryTypes.SELECT })
        .then(data1 => {
          if(data1.length!==0){  
            DataExportDate = data1[0].DataExportDate;

            models.FinishedGoodsAdjustment.findAll({ limit: 1, order: [ [ 'DataExportDate', 'DESC' ]]}).then(function(response){
                if(response.length!==0){
                
                if (DataExportDate > response[0].DataExportDate) {

                    sequelize.query("SELECT * FROM P_FinishedGoodsAdjustments where DataExportDate > "+"'"+response[0].DataExportDate.toISOString()+"'", { type: sequelize.QueryTypes.SELECT })
                        .then(data2 => {

                            if (data2.length != 0) {
                                models.FinishedGoodsAdjustment.findAll().then(function (response) {
                                    if (response.length != 0) {
                                        console.log("--------> Start InventoryItem Sync..")
                                        startMigrate(data2);
                                    } else {
                                        console.log("--------> Start InventoryItem Migrate")
                                        startMigrate(data2);
                                    }
                                });
                            } else {
                    ...
                    console.log("--------> InventoryItem Uptodate")
                    nextTable();
                }

            }else{
                sequelize.query("SELECT * FROM P_FinishedGoodsAdjustments", { type: sequelize.QueryTypes.SELECT })
...
async function startMigrate(data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        models.FinishedGoodsAdjustment.upsert(data[i]).then(async function (response) {
        ...
        if (count === data.length) {
            console.log("--------> Success InventoryItem Migrated!")
