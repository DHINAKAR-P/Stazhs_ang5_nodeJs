...
module.exports.migrateSalesOrderItems = function () {
    ...
    sequelize.query("SELECT TOP 1 * FROM P_SalesOrderItems ORDER BY DataExportDate DESC",
        { type: sequelize.QueryTypes.SELECT })
        .then(data1 => {
        if(data1.length!==0){
            DataExportDate = data1[0].DataExportDate;

            models.SalesOrderItems.findAll({ limit: 1, order: [ [ 'DataExportDate', 'DESC' ]]}).then(function(response){
            if(response.length!==0){    
                if (DataExportDate > response[0].DataExportDate) {

                    sequelize.query("SELECT * FROM P_SalesOrderItems where DataExportDate > "+"'"+response[0].DataExportDate.toISOString()+"'", { type: sequelize.QueryTypes.SELECT })
                        .then(data2 => {

                            if (data2.length != 0) {
                                models.SalesOrderItems.findAll().then(function (response) {
                                    if (response.length != 0) {
                                        console.log("--------> Start SalesOrderItems Sync..")
                                        startMigrate(data2);
                                    } else {
                                        console.log("--------> Start SalesOrderItems Migrate")
                                        startMigrate(data2);
                                    }
                                });
                            } else {
                    ...
                    console.log("--------> SalesOrderItems Uptodate")
                    nextTable();
                }

            }else{
                sequelize.query("SELECT * FROM P_SalesOrderItems", { type: sequelize.QueryTypes.SELECT })
...
async function startMigrate(data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        models.SalesOrderItems.upsert(data[i]).then(async function (response) {
        ...
        if (count === data.length) {
            console.log("--------> Success SalesOrderItems Migrated!")
